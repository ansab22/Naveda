@php
    ...
    // Database se sirf text aur link lo (agar diye ho)
    $dbItems = $contact['items'] ?? [];

    // Static icons + fallback structure
    $defaultItems = [
        [
    ...
    // Merge logic: icon hamesha static rahega
    $items = [];
    foreach ($defaultItems as $index => $item) {
        $items[] = [
            'icon' => $item['icon'],
            'text' => $dbItems[$index]['text'] ?? $item['text'],
            'link' => $dbItems[$index]['link'] ?? $item['link'],
        ];
    }
@endphp
            ...
            <div class="flex flex-col gap-2 lg:gap-6">
                @foreach($items as $item)
                    <div class="flex gap-3 w-full items-center">
                        <i class="{{ $item['icon'] }} text-[12px] lg:text-[22px] text-[#c4cffa]"></i>
                        @if(!empty($item['link']))
