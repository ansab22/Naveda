    ...
    $data = Content::getData('home.heroHome');
    @endphp
    ...
    <div class="flex flex-col lg:flex-row items-center justify-between gap-10">
        ...
        <div class="w-full lg:w-1/2 flex flex-col gap-3 lg:gap-6">
            ...
            <div class="flex flex-wrap gap-3 lg:gap-6 pt-2 lg:pt-4">
                ...
                <div class="flex items-center gap-2 text-[10px] lg:text-sm font-semibold text-gray-800">
                    <i class="fa-solid fa-circle-check text-green-300"></i> {{ $feature }}
                </div>
                @endforeach
                @else
                {{-- Default Features --}}
        ...
        : asset('/images/hero.png') }}"
                ...
                <div class="absolute bottom-10 left-21 bg-white rounded-xl shadow-md px-3 py-1 md:px-4 md:py-3 flex items-center gap-3 w-fit text-sm">
                    <div class="bg-green-900 text-white p-2 rounded-full flex items-center justify-center text-sm md:text-xl leading-none">
                        <i class="fa-solid fa-house"></i>
                    </div>
                    <div>
                        <p class="text-gray-800 font-medium text-[10px] md:text-sm">
                            {{ $data['overlay_title'] ?? 'Revolutionizing healthcare,' }}
                        </p>
                        <p class="text-gray-600 text-[10px] md:text-sm">
                            {{ $data['overlay_subtitle'] ?? 'one patient at a time' }}
                        </p>
                    </div>
                </div>
            </div>
